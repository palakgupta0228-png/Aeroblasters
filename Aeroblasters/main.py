...
import random
import pygame
from login_screen import login_menu
from auth import (
    update_highscore,
    get_highscore,
    get_skin,
...
from skin_selection import skin_selection_screen
...
player_highscore = get_highscore(current_user)
# Load previously selected skin
selected_skin = get_skin(current_user)
...
FPS = 60
# Open Aircraft Hangar
selected_skin = skin_selection_screen(win, current_user)
# COLORS **********************************************************************
...
highscore_font = pygame.font.SysFont("Arial", 18)
player_font = pygame.font.SysFont("Arial", 16)
...
bg = Background(win)
p = Player(144, HEIGHT - 100, selected_skin)
...
while running:
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYUP and game_page:
		...
		if event.type == pygame.KEYDOWN and home_page:
			if event.key == pygame.K_RETURN:
				home_page = False
				game_page = True
				click_fx.play()
	...
	if level_page:
		...
		win.blit(
			level_surface,
			(
				WIDTH//2 - level_surface.get_width()//2,
				HEIGHT//2 - 40
			)
		)
		...
		win.blit(
			msg_surface,
			(
				WIDTH//2 - msg_surface.get_width()//2,
				HEIGHT//2 + 20
			)
		)
	...
	if score_page:
		...
		highscore_surface = level_font.render(
			f"High Score: {player_highscore}",
			True,
			WHITE
		)

		win.blit(highscore_surface, (80, 320))
	...
	if game_page:
		...
		if score >= next_level_score:
			...
			plane_frequency = int(plane_frequency * 0.85)  # 15% faster each level

			if plane_frequency < 700:
				plane_frequency = 700  # hard minimum limit

			# reset timers and enemies
			enemy_group.empty()
			enemy_bullet_group.empty()
			player_bullet_group.empty()
			explosion_group.empty()
		...
		if delta_time >= plane_frequency:
			...
			e = Enemy(x, -150, enemy_type)
			enemy_group.add(e)

			# extra enemy spawn chance at higher levels
			if level >= 3 and random.random() < 0.3:
				x2 = random.randint(10, WIDTH - 100)
				e2 = Enemy(x2, -150, enemy_type)
				enemy_group.add(e2)
		...
		if p.alive:
			...
			for bullet in player_bullet_group:
				...
				planes_hit = pygame.sprite.spritecollide(
					bullet,
					enemy_group,
					False
				)
			...
			player_collide = pygame.sprite.spritecollide(
				p,
				enemy_group,
				True
			)
		...
		player_text = player_font.render(
			f"Player: {current_user}",
			True,
			WHITE
		)

		win.blit(player_text, (10, 50))
		...
		highscore_text = level_font.render(
			f"High Score: {player_highscore}",
			True,
			WHITE
		)

		win.blit(highscore_text, (10, 110))
